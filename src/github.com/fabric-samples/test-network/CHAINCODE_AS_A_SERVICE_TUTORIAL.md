...
You should be able to see the contract starting in the monitoring window. There will be two containers running, one for hosp1 and one for hosp2. The container names contain the organization/peer and the name of the chaincode.
...
```bash
# Environment variables for Hosp1

export CORE_PEER_TLS_ENABLED=true
export CORE_PEER_LOCALMSPID="Hosp1MSP"
export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/peerOrganizations/hosp1.arez/tlsca/tlsca.hosp1.arez-cert.pem
...
If you don't have `jq` installed omit `| jq`.  The metadata shows the details of the deployed contract and is JSON, so jq makes it easier to read.  You can repeat the above commands for hosp2 to confirm that is working.
...
In the peer's environment configuration we then set for hosp1's peer1
