...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Orderer Hosp Identities"
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    # COMPOSE_EDY_CA=compose-orderer2-ca.yaml -f compose/${COMPOSE_EDY_CA}
...
function networkUp() {
  ...
  # COMPOSE_EDY_ORDERER2=compose-orderer2.yaml
...
# call the script to create the channel, join the peers of hosp1 and hosp2,
...
function createChannel() {
  ...
  # cp /mnt/d/go/src/github.com/fabric-samples/test-network/organizations/peerOrganizations/hosp1.arez/connection-hosp1.json  /mnt/d/java projects/hyperLedgerServer/src/main/resources/cp/connection-org1.json
}
...
function networkDown() {
  ...
  # COMPOSE_EDY_FILES="-f compose/compose-orderer2-ca.yaml -f compose/compose-orderer-ca.yaml down"
  # stop org3 containers also in addition to hosp1 and hosp2, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.eurisko docker_peer0.hosp1.arez docker_peer0.hosp2.rizk
